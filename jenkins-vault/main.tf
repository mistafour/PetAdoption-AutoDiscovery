...
resource "aws_acm_certificate" "pet-acm-cert" {
...
resource "aws_route53_record" "acm_validation_record" {
  for_each = {
    for dvo in aws_acm_certificate.pet-acm-cert.domain_validation_options : dvo.domain_name => {
...
resource "aws_elb" "elb_jenkins" {
  ...
  listener {
    ...
    lb_port            = 443
    lb_protocol        = "HTTPS"
    ssl_certificate_id = aws_acm_certificate.pet-acm-cert.arn
  }
  health_check {
    healthy_threshold   = 3
...
resource "aws_route53_record" "vault-record" {
  zone_id = data.aws_route53_zone.pet-acp-zone.id
